...
const { gmailAccessToken } = require("../lib/google");
...
async function sendOne({ req, accessToken, campaignId, recipient, subject, plainBody, senderEmail }) {
  ...
  await gmailSupabaseRest("crm_email_messages", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ id: messageId, campaign_id: campaignId, sender_email: senderEmail, recipient_email: recipient.email, recipient_name: recipient.name, subject, status: "queued", open_token: openToken, unsubscribe_token: unsubscribeToken, created_by: req.crmUser.name }) });
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const senderEmail = req.crmUser.email.toLowerCase();
